...
import { useEffect, useState } from "react";
  ...
  //{e} from home/return/allPosts/key
const Post = ({ e }) => { 
  ...
  const handleOpenMenu = (event) => {
    dispatch(addPostId(e._id)); //storing Id in post
    dispatch(toggleMyMenu(event.currentTarget));
  };


  const checkIsAdmin = () => {
    //if adminId in post & watcherId matches 
  ...
  useEffect(() => {
    if (e && myInfo) {
      checkIsAdmin();//if e & myinfo true then call this
    }
  }, [e, myInfo]);
...
{/* <<if admin then this icon otherwise that >> */}
          {isAdmin ? (
