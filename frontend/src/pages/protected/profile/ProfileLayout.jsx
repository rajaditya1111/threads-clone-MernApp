...
const ProfileLayout = () => {
  ...
  const params = useParams(); //from router, to get params from url

  const { data } = useUserDetailsQuery(params?.id); //Redux
  const [followUser, followUserData] = useFollowUserMutation(); //Redux

  const { darkMode, myInfo } = useSelector((state) => state.service); //Redux

  const [myAccount, setMyAccount] = useState(); //Redux
  const [isFollowing, setIsFollowing] = useState(); //Redux


  //diff. breakpoints for responsiveness
